let quoteController = function (Quote) {
    ...
    let post = function (req, res) {
        ...
        let quote = new Quote(req.body);                                                            //Create a new mangoose-instance of quote-model
        ...
        if (!req.body.quote) {                                                                      //Als titel niet bestaat in req.body
            ...
            quote.save();                                                                           //Saves Quote made in Postman in mango-db
            res.status(201);                                                                        //Send status 201 means created
            res.send(quote);                                                                        //Sending quote back so quote-id is available to the client who called our API.                                                    
        }
    }
    ...
    let get = function (req, res) {
        ...
        //Quote is onze model, vind het en doe een callback.
